...
$(document).ready(function () {
    ...
    database.ref().on("value", function(snapshot) {
        database.ref().set({timesClicked: snapshot.val().timesClicked})
        $("#bored-text").text(snapshot.val().timesClicked);
    })
});
...
function ConfigureButtons() {            
    ...
    $("#bored-ball").click(function() {
        ...
        database.ref().once("value", function(snapshot) {
            ...
            let totalClicks = 1 + snapshot.val().timesClicked;
            database.ref().set({timesClicked: totalClicks});
            ...
            if ((totalClicks % 25) === 0) {
                let newColor = "#";
                for (let i = 0; i < 6; i++) {
                    newColor += colorVals[Math.floor(Math.random() * colorVals.length)];
                }
